...
while game:
    ...
    for e in pygame.event.get():
        ...
        # Обработка событий в игровой сцене
        if scene == 1:
            # Стрельба
            ...
            # Замедление времени
            if e.type == pygame.KEYDOWN and e.key == pygame.K_SPACE and player.power > 0.9:
                for en in enemy_group:
                    en.speed /= 3
                time = True

            if e.type == pygame.KEYUP and e.key == pygame.K_SPACE and time:
                for en in enemy_group:
                    en.speed *= 3
                time = False

        # Обработка событий в кат-сценах
        if scene in ["c1", "c2", "c3"] and e.type == pygame.KEYDOWN and e.key == pygame.K_SPACE:
            if scene == "c1":
                scene1.num += 1
            elif scene == "c2":
                scene2.num += 1
        ...
        if e.type == pygame.MOUSEBUTTONDOWN:
            ...
            if scene == -1:
                if b_new_game.isOver(pos):
                    player.kill()
                    player = Player()
                    scene = -2
                    pygame.mixer.music.stop()
                elif b_continue2.isOver(pos):
                ...
                if b_continue.isOver(pos):
                    scene = 1
                elif b_about.isOver(pos):
                    scene = "about"
                elif b_settings.isOver(pos):
                    scene = "controls"
                elif b_menu.isOver(pos):
                    scene = -1
                    music_menu()

            elif scene in ["about", "controls"] and b_back.isOver(pos):
                scene = -3

            elif scene == -13 and b_menu2.isOver(pos):
                scene = -1
                music_menu()
    ...
    # Переходы между уровнями
        ...
        for o in walls_group:
            if o.typew == "wall":
                o.img = pygame.transform.scale(pygame.image.load("images/wall2.png"), (50, 50))
        for o in floor_list:
        ...
        for o in walls_group:
            if o.typew == "wall":
                ...
                o.img = pygame.transform.scale(pygame.image.load("images/chair2.png"), (50, 50))
            elif o.typew == "decor":
                o.img = pygame.transform.scale(pygame.image.load("images/desk2.png"), (50, 50))
        for o in floor_list:
        ...
        for o in walls_group:
            if o.typew == "wall":
                o.img = pygame.transform.scale(pygame.image.load("images/wall3.png"), (50, 50))
        for o in floor_list:
