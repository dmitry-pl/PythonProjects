...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, health, size, speed, x, y, pathfinder):
        ...
        self.detection_range = 200  # Дистанция обнаружения игрока
        self.repath_threshold = 50  # Дистанция для перерасчета пути
        
    def update(self, dis, player, walls):
        if not self.died:
            # Обработка попадания пуль
            ...
            if distance < self.detection_range:
                # Перерасчет пути только если нужно
                if (distance > self.repath_threshold or 
                    self.path_update_cooldown <= 0 or 
                    not self.path):
                    
                    self.path = self.pathfinder.find_path(
                        (self.rect.x, self.rect.y),
                        (player.rect.x, player.rect.y))
                    self.path_update_cooldown = 30  # Обновлять путь не чаще чем раз в 30 кадров
                else:
                    self.path_update_cooldown -= 1

                # Движение по пути
                if self.path and len(self.path) > 1:
                    target_x, target_y = self.path[1]  # Берем следующую точку пути
...
def is_position_free(x, y, walls):
...
def create_enemies(count, walls, pathfinder):
    enemies = pygame.sprite.Group()
    for _ in range(count):
        x, y = pathfinder.get_random_free_position(walls)
        enemies.add(Enemy(255, (50, 50), 2, x, y, pathfinder))  # Скорость 2
    return enemies

enemy_group = create_enemies(10, walls_group, pathfinder)
...
def camera_configure(camera, target_rect):
    ...
    l = min(0, l)
    l = max(-(camera.width-1500), l)
    t = max(-(camera.height-1000), t)
    t = min(0, t)
